...
df = pd.read_csv('vpnall.csv') #这里改数据集
...
for i in range(10):
    # 计算训练集和测试集的索引
    start = i * fold_size
    end = start + fold_size if i != 9 else n_samples  # 最后一折可能包含剩余的所有样本
    X_test = X.iloc[start:end]
    y_test = y.iloc[start:end]
    X_train = X.iloc[:start].append(X.iloc[end:])
    y_train = y.iloc[:start].append(y.iloc[end:])
    ...
    pr_scores.append(precision_score(y_pred_CNN, y_test))
    rc_scores.append(recall_score(y_pred_CNN, y_test))
    f1_scores.append(f1_score(y_pred_CNN, y_test))

acc_mean = np.mean(acc_scores)
pr_mean = np.mean(pr_scores)
rc_mean = np.mean(rc_scores)
